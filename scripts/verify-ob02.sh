#!/usr/bin/env bash
# OB-02 task-conditioned source selection core release gate (gate B2).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-02 implementation, fixture, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-02-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob02: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-02 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-02: add task-conditioned source selection core (B2)" ]] \
  || fail "HEAD is not the OB-02 selection commit"
required=(
  src/selection.rs src/compiler.rs src/lib.rs
  tests/ob02_selection.rs tests/fixtures/ob02-selection-golden.json
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-02 artifact: $file"
done
[[ -x scripts/verify-ob02.sh ]] || fail "verify-ob02.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-02 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-02 adds two new modules (selection.rs, compiler.rs) and registers them in
# lib.rs. The OB-01 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    src/store.rs src/error.rs src/sync.rs src/provider.rs src/http.rs | grep -q '^[+-]'; then
  fail "an OB-01 or Option A module changed"
fi
printf '%s\n' 'ok: only additive registration and new OB-02 modules'
...
assert "time" not in direct_names, "OB-02 must not add a wall-clock dependency"
PY
...
[[ "$(sha256sum tests/fixtures/ob02-selection-golden.json | awk '{print $1}')" == "$OB02_FIXTURE_SHA256" ]] \
  || fail "OB-02 golden fixture changed"
...
# --- Step 6: OB-02 and OB-01 matrices -----------------------------------------
cargo test --locked --test ob02_selection
cargo test --locked --test ob01_receipts
printf '%s\n' 'ok: OB-02 selection and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob02: all checkpoints passed'
