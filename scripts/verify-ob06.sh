#!/usr/bin/env bash
# OB-06 omission challenge and uncertainty release gate (gate B6).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-06 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-06-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob06: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-06 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-06: add omission challenge and uncertainty (B6)" ]] \
  || fail "HEAD is not the OB-06 omission challenge commit"
required=(
  src/handoff.rs src/lib.rs
  tests/ob06_omission.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-06 artifact: $file"
done
[[ -x scripts/verify-ob06.sh ]] || fail "verify-ob06.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-06 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-06 extends the OB-05 module (handoff.rs) with the negotiation portion and
# registers the doc note in lib.rs. The OB-01/OB-02/OB-03/OB-04/OB-05 modules
# and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/selection.rs src/compiler.rs src/closure.rs src/delta.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, OB-03, OB-04, OB-05, or Option A module changed"
fi
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' | grep -v '^src/handoff.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
printf '%s\n' 'ok: only additive registration and the extended OB-06 module'
...
assert "time" not in direct_names, "OB-06 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-06, OB-05, OB-04, OB-03, OB-02, and OB-01 matrices ----------
...
printf '%s\n' 'ok: OB-06 omission, OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob06: all checkpoints passed'
