#!/usr/bin/env bash
# OB-12 semantic mechanisms decision release gate (gate B12).
#
# Records the non-adoption decision for B2's named heavy mechanisms
# (embeddings, vector search, reranking): no candidate passes the frozen
# forbidden-surface audit and the pinned 320-dependency closure, so the
# decision is recorded with the demonstrated lexical baseline and B8
# evaluation results. Fails or passes; never writes evidence, never fetches
# from the network.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-12-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob12: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-12 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-12: record semantic mechanisms decision (non-adoption)" ]] \
  || fail "HEAD is not the OB-12 semantic decision commit"
required=(
  src/lib.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-12 artifact: $file"
done
[[ -x scripts/verify-ob12.sh ]] || fail "verify-ob12.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-12 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-12 is a recorded decision: no code module, no test matrix. Only the
# additive lib.rs doc note may change; every other module stays untouched.
...
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
if git diff HEAD^ HEAD -- tests/ | grep -q '^[+-]'; then
  fail "an existing test file changed (no test matrix for the non-adoption decision)"
fi
if git diff HEAD^ HEAD --name-only -- tests | grep -q '^tests/ob12_semantic.rs$'; then
  fail "tests/ob12_semantic.rs must not exist: the decision is non-adoption"
fi
printf '%s\n' 'ok: only the additive lib.rs doc note records the OB-12 decision'
...
assert "time" not in direct_names, "OB-12 must not add a wall-clock dependency"
# No semantic-mechanism dependency entered the closure.
...
# --- Step 6: demonstrated baseline (B8 eval) and the full OB matrix ---------
...
printf '%s\n' 'ok: B8 eval baseline demonstrated; OB-11..OB-01 matrices passed'
...
printf '%s\n' 'verify-ob12: all checkpoints passed'
