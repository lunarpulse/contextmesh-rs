#!/usr/bin/env bash
# OB-11 recipient capability modeling release gate (gate B11).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-11 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-11-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob11: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-11 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-11: add recipient capability modeling (B11)" ]] \
  || fail "HEAD is not the OB-11 capability commit"
required=(
  src/capability.rs src/lib.rs
  tests/ob11_capability.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-11 artifact: $file"
done
[[ -x scripts/verify-ob11.sh ]] || fail "verify-ob11.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-11 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-11 adds the capability module (src/capability.rs) and registers the doc
# note in lib.rs. The OB-01..OB-10 modules and all Option A modules must be
# untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/compiler.rs src/closure.rs src/delta.rs src/handoff.rs \
    src/repair.rs src/eval.rs src/selection.rs src/summary.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, OB-03, OB-04, OB-05, OB-06, OB-07, OB-08, OB-09, OB-10, or Option A module changed"
fi
if git diff HEAD^ HEAD -- src/capability.rs | grep -qE '^-[^-]'; then
  fail "the OB-11 work module (capability.rs) contains deletions vs its parent"
fi
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' | grep -v '^src/capability.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
printf '%s\n' 'ok: only additive registration and the new OB-11 capability module'
...
assert "time" not in direct_names, "OB-11 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-11, OB-09, OB-10, OB-08, OB-07, OB-06, OB-05, OB-04, OB-03, OB-02, OB-01
...
printf '%s\n' 'ok: OB-11 capability, OB-09 summaries, OB-10 sufficiency, OB-08 eval, OB-07 repair, OB-06 omission, OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob11: all checkpoints passed'
