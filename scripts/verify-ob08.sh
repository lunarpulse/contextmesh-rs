#!/usr/bin/env bash
# OB-08 comprehension and task-performance evaluation release gate (gate B8).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-08 implementation, tests, frozen manifest, and evidence to
# be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-08-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob08: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-08 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-08: add comprehension and task-performance evaluation (B8)" ]] \
  || fail "HEAD is not the OB-08 comprehension evaluation commit"
required=(
  src/eval.rs src/lib.rs
  tests/ob08_eval.rs
  tests/common/mod.rs
  tests/fixtures/ob08-eval-manifest.json
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-08 artifact: $file"
done
[[ -x scripts/verify-ob08.sh ]] || fail "verify-ob08.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-08 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-08 adds the eval module (src/eval.rs), registers the doc note in lib.rs,
# extends the shared test helper allowances, and freezes the eval manifest.
# The OB-01..OB-07 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/handoff.rs src/repair.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, OB-03, OB-04, OB-05, OB-06, OB-07, or Option A module changed"
fi
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' | grep -v '^src/eval.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
printf '%s\n' 'ok: only additive registration and the new OB-08 eval module'
...
assert "time" not in direct_names, "OB-08 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-08, OB-07, OB-06, OB-05, OB-04, OB-03, OB-02, and OB-01 ----
...
printf '%s\n' 'ok: OB-08 eval, OB-07 repair, OB-06 omission, OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob08: all checkpoints passed'
