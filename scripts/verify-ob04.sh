#!/usr/bin/env bash
# OB-04 recipient-known-history delta release gate (gate B4).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-04 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-04-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob04: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-04 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-04: add recipient-known-history delta (B4)" ]] \
  || fail "HEAD is not the OB-04 delta commit"
required=(
  src/delta.rs src/lib.rs
  tests/ob04_delta.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-04 artifact: $file"
done
[[ -x scripts/verify-ob04.sh ]] || fail "verify-ob04.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-04 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-04 adds one new module (delta.rs) and registers it in lib.rs. The
# OB-01/OB-02/OB-03 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/selection.rs src/compiler.rs src/closure.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, OB-03, or Option A module changed"
fi
printf '%s\n' 'ok: only additive registration and the new OB-04 module'
...
assert "time" not in direct_names, "OB-04 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-04, OB-03, OB-02, and OB-01 matrices --------------------------
...
printf '%s\n' 'ok: OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob04: all checkpoints passed'
