#!/usr/bin/env bash
# OB-10 minimal-sufficient-context computation release gate (gate B10).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-10 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-10-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob10: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-10 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-10: add minimal-sufficient-context computation (B10)" ]] \
  || fail "HEAD is not the OB-10 minimal-sufficient-context commit"
required=(
  src/selection.rs src/lib.rs
  tests/ob10_sufficient.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-10 artifact: $file"
done
[[ -x scripts/verify-ob10.sh ]] || fail "verify-ob10.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-10 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-10 extends the OB-02 module (selection.rs) with the sufficiency/minimality
# claim discipline and registers the doc note in lib.rs. The OB-01, OB-03..
# OB-08 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/repair.rs src/eval.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-03, OB-04, OB-05, OB-06, OB-07, OB-08, or Option A module changed"
fi
if git diff HEAD^ HEAD -- src/selection.rs | grep -qE '^-[^-]'; then
  fail "the OB-10 work module (selection.rs) contains deletions vs its parent"
fi
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' | grep -v '^src/selection.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
printf '%s\n' 'ok: only additive registration and the extended OB-10 claim module'
...
assert "time" not in direct_names, "OB-10 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-10, OB-08, OB-07, OB-06, OB-05, OB-04, OB-03, OB-02, OB-01 --
...
printf '%s\n' 'ok: OB-10 sufficiency, OB-08 eval, OB-07 repair, OB-06 omission, OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob10: all checkpoints passed'
