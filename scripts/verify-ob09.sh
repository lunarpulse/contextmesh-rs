#!/usr/bin/env bash
# OB-09 hierarchical and project summaries release gate (gate B9).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-09 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-09-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob09: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-09 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-09: add hierarchical and project summaries (B9)" ]] \
  || fail "HEAD is not the OB-09 summaries commit"
required=(
  src/summary.rs src/lib.rs
  tests/ob09_summaries.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-09 artifact: $file"
done
[[ -x scripts/verify-ob09.sh ]] || fail "verify-ob09.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-09 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-09 adds the summary module (src/summary.rs) and registers the doc note in
# lib.rs. The OB-01..OB-08, OB-10 modules and all Option A modules must be
# untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/repair.rs src/eval.rs src/selection.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, OB-03, OB-04, OB-05, OB-06, OB-07, OB-08, OB-10, or Option A module changed"
fi
if git diff HEAD^ HEAD -- src/summary.rs | grep -qE '^-[^-]'; then
  fail "the OB-09 work module (summary.rs) contains deletions vs its parent"
fi
changed="$(git diff HEAD^ HEAD --name-only -- src | grep -v '^src/lib.rs$' | grep -v '^src/summary.rs$' || true)"
[[ -z "$changed" ]] || fail "unexpected source module change: $changed"
printf '%s\n' 'ok: only additive registration and the new OB-09 summary module'
...
assert "time" not in direct_names, "OB-09 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-09, OB-10, OB-08, OB-07, OB-06, OB-05, OB-04, OB-03, OB-02, OB-01
...
printf '%s\n' 'ok: OB-09 summaries, OB-10 sufficiency, OB-08 eval, OB-07 repair, OB-06 omission, OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob09: all checkpoints passed'
