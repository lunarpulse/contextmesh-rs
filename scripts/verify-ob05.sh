#!/usr/bin/env bash
# OB-05 state-bound handoff validity release gate (gate B5).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-05 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-05-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob05: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-05 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-05: add state-bound handoff validity (B5)" ]] \
  || fail "HEAD is not the OB-05 handoff commit"
required=(
  src/handoff.rs src/lib.rs
  tests/ob05_validity.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-05 artifact: $file"
done
[[ -x scripts/verify-ob05.sh ]] || fail "verify-ob05.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-05 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-05 adds one new module (handoff.rs) and registers it in lib.rs. The
# OB-01/OB-02/OB-03/OB-04 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
  ...
  fail "an OB-01, OB-02, OB-03, OB-04, or Option A module changed"
fi
printf '%s\n' 'ok: only additive registration and the new OB-05 module'
...
assert "time" not in direct_names, "OB-05 must not add a wall-clock dependency"
PY
...
# --- Step 6: OB-05, OB-04, OB-03, OB-02, and OB-01 matrices ------------------
...
printf '%s\n' 'ok: OB-05 validity, OB-04 delta, OB-03 closure, OB-02 selection, and OB-01 receipt matrices passed'
...
printf '%s\n' 'verify-ob05: all checkpoints passed'
