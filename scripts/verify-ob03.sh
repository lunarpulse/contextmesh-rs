#!/usr/bin/env bash
# OB-03 dependency closure and critical-risk coverage release gate (gate B3).
#
# Fails or passes; never writes evidence, never fetches from the network.
# Requires the OB-03 implementation, tests, and evidence to be committed.
set -euo pipefail
...
readonly EVIDENCE="_bmad-output/verification-artifacts/ob-03-evidence.md"

cd "$ROOT"
export CARGO_NET_OFFLINE=true
fail() { printf 'verify-ob03: FAIL %s\n' "$*" >&2; exit 1; }

# --- Step 1: clean worktree and committed OB-03 artifacts -------------------
[[ -z "$(git status --porcelain)" ]] || fail "worktree is not clean"
[[ "$(git log -1 --format=%s)" == "OB-03: add dependency closure and critical-risk coverage (B3)" ]] \
  || fail "HEAD is not the OB-03 closure commit"
required=(
  src/closure.rs src/lib.rs
  tests/ob03_closure.rs
  tests/common/mod.rs
...
for file in "${required[@]}"; do
  [[ -e "$file" ]] || fail "missing OB-03 artifact: $file"
done
[[ -x scripts/verify-ob03.sh ]] || fail "verify-ob03.sh is not executable"
printf '%s\n' 'ok: clean worktree, OB-03 artifacts committed at HEAD'

# --- Step 2: additive-only changes ------------------------------------------
# OB-03 adds one new module (closure.rs) and registers it in lib.rs. The
# OB-01/OB-02 modules and all Option A modules must be untouched.
...
if git diff HEAD^ HEAD -- src/receipt.rs src/crypto.rs src/cli.rs src/model.rs \
    ...
    src/selection.rs src/compiler.rs | grep -q '^[+-]'; then
  fail "an OB-01, OB-02, or Option A module changed"
fi
printf '%s\n' 'ok: only additive registration and the new OB-03 module'
...
assert "time" not in direct_names, "OB-03 must not add a wall-clock dependency"
PY
...
printf '%s\n' 'verify-ob03: all checkpoints passed'
