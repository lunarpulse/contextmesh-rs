# OC-04 4G+ — Real Human-Gold Corpus Preparation Plan (v5.4)

**Status:** APPROVED — v5.4 PRE-LABEL revision FROZEN (founder approval 2026-09-06, Discord msg 1545922919769702450: "승인") · **Date:** 2026-09-06
...
| Stratum assignment (BLOCKER-11 fix, round-2 executable form; **redefined v5.4**) | **v5.4 strata (PRE-LABEL, §2.4 clean):** `terminal_high_tokens`, `terminal_low_tokens`, `unterminated` — assigned deterministically from pre-label transcript state (terminal status, `input_tokens` median split, cutoff epoch 1787280000.0 = 2026-08-21 snapshot date). **Rationale (measured 2026-09-06, §2.4-clean):** the post-cutoff live frame (236 sessions, 234 eligible at ≥4 msgs) has 0 sessions with recorded cost > 0 (subscription-included API; cost_status mostly `unknown`/`included`) — the v5.3 cost-based strata have ZERO separating power in the real frame, while `input_tokens` (median 43,985, range 9–8.6M) and terminal status carry the §3 "usage-intensity proxy" intent. `strict_all_gold_tf0` CANNOT be known pre-label: the oversupply pool size is **pre-declared by formula in the sampling manifest** — `oversupply_N = ceil(target_stratum_N / base_rate_estimate − target_stratum_N)` where `base_rate_estimate` comes from the Stage-0 simulation's strict-stratum yield estimate (the manifest states the estimate and its source). Labeled blind, then assigned by the frozen rule (all gold candidates lexical-TF=0) AFTER labeling, NO opportunistic replacement. **Success criterion:** if assigned yield < target_stratum_N, the stratum is recorded SHORT and the gate is underpowered-for-stratum — disclosed, never swapped.
...
### v5.3 → v5.4 (2026-09-06, §3 stratum redefinition — PRE-LABEL, §2.4 clean)
- **FOUNDER APPROVAL (2026-09-06, Discord msg 1545922919769702450 "승인")**: v5.4 frozen — stratum redefinition, 2 disclosed SHORT strata (low 5, unterminated 1 families), family round-robin draw, family-majority stratum rule, corpus 192 + pilot 16 manifests, determinism/privacy verification PASS. Commit/push authorized.
- **Trigger**: founder instruction "A 진행해" (Discord trigger 1545907950256660561) to execute Option A (stratum redefinition) from the 2026-09-05 pilot-start report (@session:default/20260905_125816_0ee16c84, msg 1545835456298505) — the frozen v5.3 cost-based strata were found to have ZERO separating power in the real sampling frame.
- **Measured evidence (read-only, content-free)**: post-cutoff live frame (cutoff = OC-00.5 snapshot date 2026-08-21, epoch 1787280000.0): 236 sessions, 234 eligible at ≥4 msgs; `actual_cost_usd > 0` in 0 eligible sessions (subscription-included API; cost_status ∈ {unknown 111, included 31, estimated 1, null 5} in the snapshot, same regime live). v5.3 strata `terminal_with_full_cost`/`terminal_with_partial_cost` would collapse to a single basket.
- **Change**: strata redefined to `terminal_high_tokens` / `terminal_low_tokens` (input_tokens median split, pre-label fields only) / `unterminated`. Preserves the §3 usage-intensity-proxy intent with real separating power (median 43,985 tokens, range 9–8.6M, 117/117 split).
- **Sampling design additions (deterministic, pre-label)**:
  - *Family round-robin draw*: within each stratum, sessions are drawn one-per-family per pass (deterministic shuffled family order). Plain proportional sampling concentrated 95 low-token sessions into as few as 1 family — an extraction artifact that would destroy the family-bootstrap effective-N. Round-robin spreads the sample across the maximum real families (measured: low 11 families in-sample vs 1 without).
  - *Family-level stratum rule*: a family is ONE cluster (§3 BLOCKER-2) and mixed-stratum families are common in the real frame (10/19). Family stratum = majority over terminated members (ties → high), `unterminated` only if every member is. Per-member counting misclassified family totals.
- **Measured frame consequence (disclosed, not hidden)**: post-cutoff `unterminated` = 2 sessions (0 eligible at ≥4 msgs) → the effective design is 2 strata (high 116 / low 116, exact 50/50). Achieved per-stratum families in the n=192 sample: high 13 (PASS ≥12), low 5 (SHORT 7), unterminated 1 (SHORT 11) — recorded in the manifest as underpowered-for-stratum, never swapped. Frame has 19 distinct parent families total; the low stratum's family scarcity is a property of the real frame, not the sampler. Stage-0 v2 (GO, n=192) is family-cluster-based and stratum-agnostic → GO stands; the manifest pins the frame measured above.
- **No labels exist** (§2.4 clean restart). sample.py rewritten (manifest version 2), corpus + pilot(16) manifests generated and verified deterministic (double-run digests identical, `00d20dff…` / `a9b27832…`), pilot ⊆ corpus, no raw ids in committed artifacts (raw ids stay in gitignored local snapshot; HMAC key gitignored, 0600). Inspect scripts removed in the same commit.
