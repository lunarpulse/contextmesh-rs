...
| OC01-P07 | `issue` order is validate -> load/verify every referenced event/context/current refs -> derive ID -> sign -> independent self/store verify. | `issue_executes_fail_closed_steps_in_exact_order` | `contextmesh-salience/src/outcome.rs`; `contextmesh-salience/tests/oc01_dag.rs` | Static source audit anchors exact implementation order; public author/missing/stale boundary tests prove fail-closed return behavior without an added production instrumentation seam. |
| OC01-P08 | Publicly injectable validation, missing-event, and stale-input issuance failures return no artifact/report and do not mutate observable store state. Internal sign/self-verify fault injection is not exposed by OC-01 v1 public APIs and is covered by the P07 implementation-order audit plus structural verification vectors. | `issue_returns_no_artifact_and_never_mutates_store_on_any_failure` | `contextmesh-salience/tests/oc01_dag.rs`; `contextmesh-salience/tests/oc01_crypto.rs` | Each public failure call is bounded by exact before/after local+remote snapshots and fixture-event presence; no partial artifact/report exists. |
...
| OC01-D03 | Missing event fails closed. | `every_event_role_rejects_missing_event` | `contextmesh-salience/tests/oc01_dag.rs` | Each event role missing returns `missing-event`, no verification object/artifact. |
| OC01-D04 | Cross-context event fails closed. | `every_event_role_rejects_cross_context_event` | `contextmesh-salience/tests/oc01_dag.rs` | Each event role with another ContextId returns `context-mismatch`. |
...
| OC01-D09 | A structurally valid embedded snapshot differing from a fresh capture—public API add/name/head/peer movement or externally observable removal—is stale input. A direct invalid caller-supplied fingerprint rejects as `id-mismatch` before `verify_current_inputs`. Public removal-transition coverage is deferred because OC-01 v1 `Store` exposes no removal API. | `current_input_snapshot_change_matrix_returns_stale_input` | `contextmesh-salience/tests/oc01_dag.rs` | Public local/remote add/move and fresh-capture difference cases return `stale-input`; direct invalid-fingerprint case returns `id-mismatch`; future public removal API reopens transition coverage. |
